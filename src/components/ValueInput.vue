<template>
  ...
  >
    <template #addonBefore>
      <a-select v-model:value="type" style="width: 52px;">
        <a-select-option value="1">{{
          t("validate.types.string")
        }}</a-select-option>
        <a-select-option value="2">{{
          t("validate.types.number")
        }}</a-select-option>
        <a-select-option value="3">{{
          t("validate.types.boolean")
        }}</a-select-option>
      </a-select>
    </template>
...
<style>
._fd-value-input .ant-input-group-addon {
  padding: 0 0 0 0 !important;
  width: 56px;
}
._fd-value-input .ant-input-sm,
._fd-value-input .ant-select-selector {
  padding: 0 2px !important;
  font-size: 12px !important;
}

._fd-value-input .a-select,
._fd-value-input .a-select-wrapper {
  height: 100%;
}
</style>
