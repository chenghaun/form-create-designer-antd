<template>
  <div class="_fd-size-input">
    <template v-if="unit[idx] === 'auto'">
      <a-button :size="size" style="width: 150px;" @click="changeType()">{{
        unit[idx]
      }}</a-button>
    </template>
    <template v-else>
  ...
  methods: {
    parseValue() {
      if (this.modelValue !== "auto") {
        ...
        this.num = isNull(this.modelValue)
          ? null
          : parseFloat(this.modelValue || 0);
      } else {
        this.idx = 0;
        this.num = 0;
      }
    ...
    submit() {
      this.oldValue = !isNull(this.num)
        ? "" + this.num + this.unit[this.idx]
        : "";
