<template>
  ...
  >
    <a-tag color="success" effect="plain" disable-transitions>
...
export default defineComponent({
  ...
  computed: {
    ...
    menus() {
      ...
      return menus.filter(
        (menu) => this.designer.setupState.hiddenItem.indexOf(menu.name) === -1
      );
    },
  },
  methods: {
    handleCommand(item) {
      ...
      if (newRule.field && activeRule.field) {
        ["title", "info", "field", "validate", "control", "$required"].forEach(
          (k) => {
